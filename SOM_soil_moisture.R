...
fili <- list.files(path = diri, pattern = "^SM_sample")

# get file names of the valid locations 
fvld <- as_tibble(read_csv("/Users/tomokokoyama/Data/LDM/sm_valid_location.csv",
                           show_col_types = FALSE))　
valid_id <- fvld$file_id
...
# 173/(4x4) = 10.81
set.seed(100)
all.grid <- somgrid(xdim = 4, ydim = 4, topo = "hexagonal")

# make a SOM model
set.seed(100)
all.model <- som(all.train, 
                 grid = all.grid, 
                 rlen = 500,               # times to present data
...
out <- data.frame(all, cluster = clust$cluster[all.model$unit.classif],
  ...
  mutate(file_id = valid_id) %>%
  arrange(cluster, mn4)

write.csv(out, paste0(diri,"ERA5_SM_6clusters.csv"), quote=FALSE, row.names=FALSE)
